...
describe('filterDegenerate', () => {
  ...
  it('rejects static patterns (all bullets same position)', () => {
    ...
    const results = [...filterDegenerate([staticPair], { bulletCount: 10, timeSteps: 50 })];
    expect(results.length).toBe(0);
  });
  ...
  it('rejects identical trajectories (all bullets follow same path)', () => {
    ...
    const results = [...filterDegenerate([identicalPair], { bulletCount: 10, timeSteps: 50 })];
    expect(results.length).toBe(0);
  });
  ...
  it('returns pair with trajectory data for accepted patterns', () => {
    ...
    const results = [...filterDegenerate([pair], { bulletCount: 10, timeSteps: 50, dt: 0.1 })];
